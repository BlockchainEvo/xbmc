...
{  
  ...
  // Temporary stream - don't delete, it is freed at the by the TagLib::File destructor
  TagLibVFSStream *tagLibVFSStream = new TagLibVFSStream(strFileName, true); 
  if (strExtension == "ape")
    file = apeFile = new APE::File(tagLibVFSStream);
  else if (strExtension == "asf" || strExtension == "wmv" || strExtension == "wma")
    file = asfFile = new ASF::File(tagLibVFSStream);
  else if (strExtension == "flac")
    file = flacFile = new FLAC::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
  else if (strExtension == "it")
    file = itFile = new IT::File(tagLibVFSStream);
  else if (strExtension == "mod" || strExtension == "module" || strExtension == "nst" || strExtension == "wow")
    file = modFile = new Mod::File(tagLibVFSStream);
  else if (strExtension == "mp4" || strExtension == "m4a" || 
           strExtension == "m4r" || strExtension == "m4b" || 
           strExtension == "m4p" || strExtension == "3g2")
    file = mp4File = new MP4::File(tagLibVFSStream);
  else if (strExtension == "mpc")
    file = mpcFile = new MPC::File(tagLibVFSStream);
  else if (strExtension == "mp3" || strExtension == "aac")
    file = mpegFile = new MPEG::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
  else if (strExtension == "s3m")
    file = s3mFile = new S3M::File(tagLibVFSStream);
  else if (strExtension == "tta")
    file = ttaFile = new TrueAudio::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
  else if (strExtension == "wv")
    file = wvFile = new WavPack::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
  else if (strExtension == "xm")
    file = xmFile = new XM::File(tagLibVFSStream);
  else if (strExtension == "ogg")
    file = oggVorbisFile = new Ogg::Vorbis::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
  else if (strExtension == "oga") // Leave this madness until last - oga container can have Vorbis or FLAC
  {
    file = oggFlacFile = new Ogg::FLAC::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
    if (!file->isValid())
    {
      delete file;
      // Deleting the file also deletes the VFSStream, create a new one
      TagLibVFSStream *tagLibVFSStream = new TagLibVFSStream(strFileName, true); 
      file = oggVorbisFile = new Ogg::Vorbis::File(tagLibVFSStream, ID3v2::FrameFactory::instance());
    }
  }
