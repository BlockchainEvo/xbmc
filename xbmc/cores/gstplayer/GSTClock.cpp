...
int64_t CGSTClock::m_systemOffset;
int64_t CGSTClock::m_systemFrequency;
CCriticalSection CGSTClock::m_systemsection;

bool CGSTClock::m_ismasterclock;
...
double CGSTClock::GetAbsoluteClock(bool interpolated /*= true*/)
{
  CSingleLock lock(m_systemsection);
  CheckSystemClock();
...
double CGSTClock::WaitAbsoluteClock(double target)
{
  CSingleLock lock(m_systemsection);
  CheckSystemClock();
...
double CGSTClock::GetClock(bool interpolated /*= true*/)
{
  CSharedLock lock(m_critSection);
  return SystemToPlaying(g_VideoReferenceClock.GetTime(interpolated));
}

double CGSTClock::GetClock(double& absolute, bool interpolated /*= true*/)
{
...
void CGSTClock::SetSpeed(int iSpeed)
{
...
void CGSTClock::Discontinuity(double currentPts)
{
  CExclusiveLock lock(m_critSection);
...
bool CGSTClock::SetMaxSpeedAdjust(double speed)
{
...
int CGSTClock::UpdateFramerate(double fps, double* interval /*= NULL*/)
{
...
void CGSTClock::CheckSystemClock()
{
...
double CGSTClock::SystemToAbsolute(int64_t system)
{
  return DVD_TIME_BASE * (double)(system - m_systemOffset) / m_systemFrequency;
}

double CGSTClock::SystemToPlaying(int64_t system)
{
  int64_t current;
