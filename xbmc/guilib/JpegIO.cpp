...
bool CJpegIO::CreateThumbnail(const CStdString& sourceFile, const CStdString& destFile, int minx, int miny, bool rotateExif)
{
  ...
  if (!Open(sourceFile, minx, miny, NULL, NULL, false))
    return false;
...
bool CJpegIO::CreateThumbnailFromMemory(unsigned char* buffer, unsigned int bufSize, const CStdString& destFile, unsigned int minx, unsigned int miny)
{
  ...
  if (!Decode(sourceBuf,Width()*3,XB_FMT_RGB8))
  {
    delete [] sourceBuf;
    return false;
  }
  if (!CreateThumbnailFromSurface(sourceBuf, Width(), Height() , pitch, destFile))
  {
    delete [] sourceBuf;
    return false;
  }
...
bool CJpegIO::CreateThumbnailFromSurface(unsigned char* buffer, unsigned int width, unsigned int height, unsigned int pitch, const CStdString& destFile)
{
  //Encode raw data from buffer, save to destFile
  struct jpeg_compress_struct cinfo;
  struct my_error_mgr jerr;
  JSAMPROW row_pointer[1];
  unsigned char* dst = new unsigned char [(int)(width * height)]; //Initial buffer. Grows as-needed.
  ...
  else
  {
    ...
    if (m_exif_data)
      jpeg_write_marker(&cinfo, JPEG_APP0 + 1, m_exif_data, m_exif_size);
